#!/bin/bash
#Batuhan Cingöz Tarafından Kodlandı.
...
print(r.status_code, r.reason)' >> batu.py

for (( i = 0; i <= $for; i++ )); do
        clear
        python batu.py> /dev/null 2>&1 &
        echo -e '\e[31mBatuhan Cingöz Tarafından Kodlandı.)'
        ...
        cd Api/ &&
        echo "Batuhan Cingöz Tarafından KODLANMIŞTIR!" >> ok.txt
        cd .. && bash tst.sh
